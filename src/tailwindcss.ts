...
export default (api: IApi) => {
  ...
  try {
    ...
    if (semver.lt(tailwindVersion, "4.0.0")) {
      throw new Error(
        `Tailwindcss version ${tailwindVersion} is not supported, please upgrade to 4.0.0 or above`
      );
    }
  } catch (error) {
    throw new Error(
      `Can't find tailwindcss package. Please install tailwindcss first.`
    );
  }
          ...
          if (!existsSync(generatedPath)) {
            ...
            api.logger.error(
              `tailwindcss generate failed after ${
                api.config.tailwindcss?.timeout ?? 10
              } seconds, please check your tailwind.css`
            );
            process.exit(1);
          }
