...
{{#MetisUI}}
import { Dropdown, SemanticClassName } from "metis-ui";
import { MenuInfo } from "metis-ui/{{{metisuiFiles}}}/menu/interface";
{{/MetisUI}}
...
export const SelectLang: React.FC<SelectLangProps> = (props) => {
 ...
 const {
  ...
  const handleClick = onItemClick
    ? (params: MenuInfo) => onItemClick(params)
    : changeLang;
  ...
  return (
    <Dropdown menu={langMenu} placement="bottomRight" {...restProps}>
      <span className="cursor-pointer p-3 inline-flex items-center justify-center text-lg">
        {icon ? (
          icon
        ) : (
          <svg
